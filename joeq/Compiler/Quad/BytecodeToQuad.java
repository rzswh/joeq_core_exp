...
public class BytecodeToQuad extends BytecodeVisitor {
    ...
    private LinkedList<joeq.Compiler.BytecodeAnalysis.BasicBlock> regenerate;

    private HashMap<Quad, Integer> quad2bci = new HashMap<Quad, Integer>();
    ...
    public ControlFlowGraph convert() {
        ...
        regenerate = new LinkedList<joeq.Compiler.BytecodeAnalysis.BasicBlock>();
        ...
        joeq.Compiler.BytecodeAnalysis.BasicBlock first_bb = rpo.next();
        Assert._assert(first_bb == bc_cfg.getEntry());
        while (rpo.hasNext()) {
            joeq.Compiler.BytecodeAnalysis.BasicBlock bc_bb = rpo.next();
    ...
    public void traverseBB(joeq.Compiler.BytecodeAnalysis.BasicBlock bc_bb) {
        ...
        for (Iterator<Quad> i = this.quad_bb.iterator(); i.hasNext(); ) {
            Quad q = i.next();
            Integer old = this.quad2bci.remove(q);
            if (old == null) {
     ...
     */
    java.util.Map<Quad, Integer> getQuadToBytecodeMap() {
        return quad2bci;
    }
    ...
    java.util.Map<joeq.Compiler.BytecodeAnalysis.BasicBlock, AbstractState> jsr_states = new java.util.HashMap<joeq.Compiler.BytecodeAnalysis.BasicBlock, AbstractState>();
    ...
    void mergeStateWithAllExHandlers(boolean cfgEdgeToExit) {
        ...
        while (i.hasNext()) {
            joeq.Compiler.BytecodeAnalysis.ExceptionHandler eh = i.next();
            mergeStateWith(eh);
        }
    }
