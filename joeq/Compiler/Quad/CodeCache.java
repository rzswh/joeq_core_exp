...
public class CodeCache {
    ...
    protected Map<jq_Method, ControlFlowGraph> map = new HashMap<jq_Method, ControlFlowGraph>();
    protected Map<jq_Method, Map<Quad, Integer>> bcmap = new HashMap<jq_Method, Map<Quad, Integer>>();

    public static List<ControlFlowGraphVisitor> passes = new LinkedList<ControlFlowGraphVisitor>();
    ...
    public static Map<Quad, Integer> getBCMap(jq_Method m) { return cache._getmap(m); }
    ...
    protected ControlFlowGraph _get(jq_Method m) {
        ...
        if (cfg == null) {
            ...
            for (java.util.Iterator<ControlFlowGraphVisitor> i = passes.iterator(); i.hasNext(); ) {
    ...
    protected Map<Quad, Integer> _getmap(jq_Method m) {
        Map<Quad, Integer> result = bcmap.get(m);
        if (result == null) {
            if (TRACE) System.out.println("Generating quads for "+m);
            BytecodeToQuad b2q = new BytecodeToQuad(m);
