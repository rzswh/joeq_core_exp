...
public class ReachingDefs extends Problem {
    ...
    public void initialize(Graph g) {
        ...
        for (Iterator<BasicBlock> i = list.iterator(); i.hasNext(); ) {
            BasicBlock bb = i.next();
            BitString gen = new BitString(bitVectorSize);
            for (Iterator<Quad> j = bb.iterator(); j.hasNext(); ) {
                Quad q = j.next();
                ...
                int a = q.getID();
                quads[a] = q;
                for (Iterator<RegisterOperand> k = q.getDefinedRegisters().iterator(); k.hasNext(); ) {
                    Register r = k.next().getRegister();
    ...
    private void handleEdges(BasicBlock bb, List<BasicBlock> bbs, BitString gen, GenKillTransferFunction defaultTF) {
        for (Iterator<BasicBlock> k = bbs.iterator(); k.hasNext(); ) {
            BasicBlock bb2 = k.next();
    ...
    void withinBasicBlock(BitString bs, BasicBlock bb, Quad q2) {
        for (Iterator<Quad> j = bb.iterator(); ; ) {
            Quad q = j.next();
            if (q == q2) break;
            if (q.getDefinedRegisters().isEmpty()) continue;
            int a = q.getID();
