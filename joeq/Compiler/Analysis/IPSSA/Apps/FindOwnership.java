...
public class FindOwnership {
    static class SimpleOwnershipFinder implements Runnable {
        ...
        OwnershipValue processBlock(jq_Method m, BasicBlock block, OwnershipValue predValue) {
            ...
            for(Iterator<Quad> qiter = block.iterator(); qiter.hasNext();) {
                Quad q = qiter.next();
                ...
                if(q.getOperator() instanceof Operator.Move){
                    Iterator<RegisterOperand> uiter = q.getUsedRegisters().iterator();
                    Iterator<RegisterOperand> diter = q.getDefinedRegisters().iterator();
                    
                    if(!uiter.hasNext() || !diter.hasNext()) continue;
                    Operand.RegisterOperand use = (Operand.RegisterOperand)uiter.next();
                    Operand.RegisterOperand def = (Operand.RegisterOperand)diter.next();
